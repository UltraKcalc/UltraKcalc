(function () {
  ...
  var nutrientDefs = [
    { id: 'energy', key: 'Energia (kcal)', label: 'Energia', unit: 'kcal' },
    { id: 'carbs', key: 'Carboidrato total', label: 'Carboidratos', unit: 'g' },
    { id: 'protein', key: 'Prote\u00edna', label: 'Prote\u00ednas', unit: 'g' },
    { id: 'lipids', key: 'Lip\u00eddios', label: 'Lip\u00eddios', unit: 'g' }
  ];
  ...
  async function loadCompareRecords() {
    ...
    if (window.UltraKcalcCloud && window.UltraKcalcCloud.isConfigured && window.UltraKcalcCloud.isConfigured()) {
      try {
        ...
        if (user) {
          ...
          compareDataSource = unsyncedLocal.length > 0 ? 'nuvem + locais' : 'nuvem';
          setText('compareDataStatus', compareRecords.length + ' recordatorios carregados (' + compareDataSource + ')');
          return;
        }
      } catch (e) {
        console.error(e);
    ...
    setText('compareDataStatus', compareRecords.length + ' recordatorios carregados (local)');
  }

  function buildRecordLabel(session, index) {
    var participant = session && session.participantId ? session.participantId : 'Sem ID';
  ...
  function modeLabel(summary) {
    if (!summary) return '';
    if (summary.type === 'record') return 'Recordatorio individual';
    if (summary.mode === 'average') return 'Media por recordatorio';
    return 'Soma total do grupo';
  }
  ...
  function renderEnergyChart(container, summary) {
    ...
    title.textContent = 'Energia por processamento';
    ...
    legend.appendChild(makeLegendChip('naup', 'NAUP', naup, total, 'kcal'));
    legend.appendChild(makeLegendChip('aup', 'AUP', aup, total, 'kcal'));
  ...
  function renderMacroCharts(container, summary) {
    ...
    var title = document.createElement('h2');
    title.textContent = 'Macronutrientes por processamento';
    ...
    nutrientDefs.filter(function (def) {
      ...
      detail.textContent = 'NAUP: ' + formatNumber(naup, 2) + ' ' + def.unit + ' (' + formatPercent(total > 0 ? (naup / total) * 100 : 0) + ') | AUP: ' + formatNumber(aup, 2) + ' ' + def.unit + ' (' + formatPercent(total > 0 ? (aup / total) * 100 : 0) + ')';
  ...
  function renderComparison() {
    ...
    if (!summaryA || !summaryB || !compareRecords.length) {
      ...
      if (empty) {
        empty.hidden = false;
        empty.textContent = 'Salve recordatorios na calculadora ou entre na conta para comparar dados da nuvem.';
      }
      return;
    }
